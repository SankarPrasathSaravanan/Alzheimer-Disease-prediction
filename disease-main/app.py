...
# modelss = joblib.load('Alzheimer_Training_History.pkl')
modelss = load_model('Alzheimer_Trained_Model2.h5')
...
def predict_image(image_path):
    ...
    x = np.array(img.resize((128, 128)))  # Resize to model input shape
    x = x.reshape(1, 128, 128, 3)  # Add batch dimension
    res = modelss.predict_on_batch(x)  # Model prediction
    classification = np.argmax(res, axis=1)[0]  # Get class index
    confidence = res[0][classification] * 100  # Calculate confidence
    prediction = names(classification)  # Get prediction name
    return f'{confidence:.2f}% Confidence This Is {prediction}'

# Define route for Alzheimer’s disease prediction
@app.route("/predict_alzheimers", methods=['GET', 'POST'])
def predict_alzheimers():
    if request.method == 'POST':
        ...
        if file:
            ...
            return render_template("kidney_result.html", prediction_text=prediction)
    return render_template("kidney_result.html")
